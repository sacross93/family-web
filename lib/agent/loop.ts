...
/**
 * 여섯 줄은 빼면 안 된다. 앞의 둘은 안전(할 수 있는 일의 경계·바깥 글의 지위),
 * 셋째는 정직(모름을 감추지 않기), 넷째는 말투, 다섯째는 **읽기와 검색의 구분**,
 * 여섯째는 **끝까지 해내기**다.
 *
 * 다섯째가 왜 필요한가: 공급자 내장 웹검색은 **본 것을 지어내기도 한다**(실측 —
 * 유튜브 도입부 대사를 물었더니 실제와 다른 문장을 인용했다). read_url 은 우리가 받아 온
 * 글이라 그런 일이 없다. 그래서 순서를 정하고, 검색으로 안 것은 검색이라고 말하게 한다.
 *
...
const RULES = [
  ...
  "주소를 받으면 **먼저 read_url 로 직접 열어 보세요.** 웹검색은 그것이 막혔을 때만 씁니다. 웹검색으로 알아낸 것은 페이지를 직접 읽은 것이 아니므로, 답할 때 어느 쪽인지 밝히세요.",
