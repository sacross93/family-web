...
| 토큰 | 값 | 무엇 |
|---|---|---|
| `paper` | `#fdf4f8` | 페이지 바탕 (아주 옅은 핑크 종이) |
| `surface` | `#ffffff` | 내용 판 |
| `sunken` | `#fbe9f0` | 판 안에서 한 단 들어간 자리 |
| `chrome` | `#fad9e6` | **틀** — 사이드바·상단바·탭바·히어로 (연한 로즈) |
| `chrome-soft` | `#f6c9da` | 틀 위에서 한 단 뜬 자리 |
| `chrome-ink` | `#3e2f36` | 틀 위 글자 (9.7:1) |
| `chrome-faint` | `#6e5560` | 틀 위 흐린 글자 (5.2:1) |
| `line` / `line-strong` | `#f4e2ea` / `#ebd0dd` | 실선 |

틀이 밝아지면서 **예전 규칙이 세 가지 뒤집혔다.** 진한 틀을 전제로 쓴 코드가 남아 있으면
조용히 사라지는 쪽이라, 새로 그릴 때 아래를 본다.

- **틀 위에서는 어두운 쪽을 쓴다.** 탭바의 포동이 버튼처럼 눈에 띄어야 하는 것은
  `bg-chrome-ink text-chrome` 으로 **어둡게 채워** 뒤집는다(9.7:1).
  밝은 쪽으로 채우면 사라진다 — `chrome-soft` 는 틀 위에서 1.1:1 이다.
- **`ink` 세 벌을 그대로 옮기지 않는다.** 틀이 밝다고 `text-ink-faint` 를 틀 위에 쓰면
  4.3:1 로 AA 에 못 미친다. 틀 위 글자는 언제나 `chrome-ink`·`chrome-faint`.
- **반투명 흰색을 틀 위에 깔지 않는다.** 진한 틀 시절의 `bg-white/10` 은 연한 로즈 위에서
  1.0:1 — 있으나 마나다. 채움이 필요하면 흰 판(`bg-surface`, 1.3:1),
  테두리는 `border-chrome-ink/20`(1.4:1).
  CSS 에서 직접 쓸 때는 `color-mix(in srgb, var(--color-chrome-ink) 20%, transparent)`.
  `line`·`line-strong` 도 안 된다 — 둘 다 옅은 핑크라 이 로즈 위에서 1.05 / 1.11 로 사라진다.
...
- `primary` `#be4571` — 주요 액션(로즈). 흰 글자가 4.9:1.
  밝은 판에 **글자로** 쓸 때는 `primary-ink`(`#a93a63`) — `primary` 자체는 흰 판에서는
  AA 를 넘지만 들어간 자리(4.2)·틀(3.8) 위에서는 못 넘어, 어디에 놓일지 모르는 채로는 쓸 수 없다.
- `accent` `#c2691f` — **"지금 벌어지는 일" 에만.** 오늘 날짜, 남은 날, 진행.
  **글자로 쓰지 않는다**(흰 판에서 3.9:1). 밝은 판에 글자가 필요하면 `accent-ink`(`#8a4410`).
  그래픽으로 쓸 때도 틀 위에서 3.02:1 — UI 기준 3:1 에 여유가 0.02 뿐이니 값을 건드리지 말 것.
