...
`꾸미기` 는 상단 메뉴 페이지(NAV href 일치)에서만 렌더된다 — 계획 상세 같은 곳엔 없다. 그래서 `물어보기` 는 **꾸미기가 있으면 그 위(`bottom-20`), 없으면 맨 아래(`bottom-5`)** 로 내려온다. 고정 오프셋으로 두면 빈자리에 혼자 떠 있게 된다. 판정 로직은 `lib/nav.ts` 에서 공유한다.
...
**미확인**: 모델에게 넘길 때 공개 URL 로 되는지, base64 로 실어야 하는지. 프로덕션 Blob 은 공개지만 개발은 `public/uploads` 상대경로라 외부에서 못 본다. **구현 전에 실측으로 확정한다**(§16 의 탐침 방식 그대로). 실측 결과에 따라 한 함수만 달라진다.
