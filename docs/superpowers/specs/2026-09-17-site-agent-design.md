...
> **이 절은 1단계 때의 스케치다. 실제로 지은 것은 §18.4·§18.6 이고, 어긋나면 그쪽이 맞다.**
> 달라진 것: 버튼 아이콘은 `✨` 가 아니라 말풍선(`MessageCircle`)이고, 꾸미기가 뜨는 조건은
> "상단 메뉴 페이지"만이 아니라 "상단 메뉴 페이지 **그리고 관리자**"다(§18.6).
...
**기록**: `☰` 를 누르면 같은 시트 안에서 목록으로 전환된다(별도 서랍 아님 — 폰에서 층이 늘면 길을 잃는다). 항목은 제목·시각·메시지 수, 탭하면 이어서 대화.

**삭제는 항상 보이는 휴지통이다** — 처음 스펙에 적었던 "밀거나 길게 눌러 삭제"는 **취소한다**. AGENTS.md 의 "hover 로만 뜨는 액션 금지"는 숨은 제스처에도 같은 취지로 걸린다(보이지 않으면 없는 것이다). 게다가 이 저장소의 목록은 장보기·아기 링크·기념일·계획까지 전부 항상 보이는 휴지통이라, 여기만 제스처로 두면 **이 화면에서만 규칙이 다르다.** 폰에서는 늘 보이고 데스크톱에서만 hover 로 숨긴다(`opacity-100 lg:opacity-0 lg:group-hover:opacity-100`).

**지금 보고 있는 대화는 표시한다**(`bg-sunken` + `aria-current`). 어디에 있었는지 모르면 길을 잃는다. 그 대화를 지우면 화면도 새 대화로 비운다 — 안 그러면 다음에 보내는 말이 지워진 `chatId` 로 나가 막힌다.
...
- **모델이 없는 페이지를 말로 안내한다.** `/family`·`/decorations` 는 목차의 `listPath` 자리를 채우려고 둔 가상 경로라 실제 페이지가 없다. 링크는 `canVisit` 이 막지만 **글로는 "가족 페이지에서 확인할 수 있습니다" 라고 말한다**(실제 관측). 화면 쪽에서는 더 막을 방법이 없다 — 고치려면 목차가 그 둘을 페이지처럼 부르지 않아야 한다(1단계 `resources.ts` 의 일).

### 18.8 범위 밖 (3단계 이후)

화면 캡처 · 대화 검색 · 사용자별 기록 분리 · 예약 실행.
(자리만 잡아 뒀던 `view_screen` 도구는 2026-09-21에 **뺐다** — 늘 "지원 안 함" 만 돌려주면서 여섯 걸음 중 한 걸음을 먹었다. 화면 위치는 이제 안내문이 준다. 화면 캡처를 정말 하게 되면 그때 다시 만든다.)
...
## 20. 링크 읽기 (2026-09-20 승인)

> "url 을 주면 그 홈페이지를 완벽하게 읽어서 알려줘. 필요하면 스크린샷을 찍어서 시각 정보도 봐야 해.
> 유튜브 주소를 주면 스크립트를 읽어서 무슨 내용인지 알려줘."

**짓기 전에 쟀다.** 실측은 [2026-09-20-web-reading-findings.md](2026-09-20-web-reading-findings.md) 에 있고, 그 결과가 아래 설계를 강제했다.

### 20.1 실측이 뒤집은 두 가지

1. **유튜브 자막은 못 받는다.** 직접·변종·옛 엔드포인트·InnerTube 4클라이언트·**진짜 크롬 안**·공개 프록시까지 전부 HTTP 200 에 빈 몸통이다. PO 토큰이 없어서고, **브라우저를 지어도 안 풀린다.**
2. **일반 사이트는 브라우저 없이 주류 5/5 가 읽힌다.** 못 읽는 네 종류 중 렌더가 고치는 건 하나뿐이다 — 쿠팡은 렌더해도 차단, 인스타는 안 지어도 블롭에 다 있다.

**그래서 v1 에 헤드리스 브라우저를 넣지 않는다.** 무료 제약과 싸울 이유가 사라졌다.

### 20.2 계단

`read_url` 의 **겉모습은 그대로 두고 속을 계단으로 바꾼다.** 도구 수는 고정이다(AGENTS.md. 이 글을 쓸 때 5개, 2026-09-21에 껍데기였던 `view_screen` 을 빼고 **4개**) — 유튜브도 새 도구가 아니라 **주소 모양으로 갈라지는 한 갈래**다.

```
①  메타        <title> · description · og:* · canonical
②  JSON-LD     application/ld+json → @type · articleBody · description
③  본문        main/article 안쪽을 우선하고 nav·header·footer·aside 는 버린다
④  블롭        self.__next_f(App Router) · __NEXT_DATA__ · __NUXT_DATA__ · window.__NUXT__
               · __INITIAL_STATE__ · __PRELOADED_STATE__ · type="application/json"
⑤  그림        og:image · JSON-LD image · 본문 안 큰 이미지 (최대 2장)
```

위 칸에서 충분하면 아래로 안 간다. **⑤는 렌더가 아니라 페이지가 스스로 내놓은 그림이다.**

### 20.3 "스크린샷" 대신 페이지의 그림을 쓴다

사용자가 말한 건 스크린샷이지만 목적은 *시각 정보를 본다*이다. 그 목적에는 **페이지 자신의 이미지가 더 낫다.**

- `og:image` 는 애초에 "이 페이지를 한 장으로 대표하는 그림"으로 만들어진 값이다
- 스크린샷은 쿠키 배너·로그인 모달·광고가 절반을 덮은 그림이 되기 쉽다
- 브라우저 한 벌이 필요 없다 — 무료 티어 제약이 사라진다
- §19 에서 지은 `input_image` 배관을 그대로 쓴다

**한계도 적는다**: 레이아웃 자체가 내용인 페이지(요금표 이미지가 아닌 표, 지도)는 이걸로 못 본다. 그 경우가 실제로 걸리면 그때 ⑥으로 렌더를 붙인다 — 실측 문서에 "렌더가 고치는 건 네 종류 중 하나"라고 적힌 그 자리다.

이미지는 **읽기와 같은 차단 규칙**을 통과해야 한다(사설망·비 http(s) 금지). 그림도 fetch 다.

### 20.4 유튜브

자막이 아니라 **받을 수 있는 것**으로 답한다:

- 제목 · 채널 · 길이 · 조회수 (`ytInitialPlayerResponse.videoDetails`)
- **설명** (`shortDescription`) — 실측 3,416자
- **챕터** (`ytInitialData` 의 `chapterRenderer`) — 실측 12개. 사실상 목차다
- 폴백: `oembed`(제목·채널만)

**자막을 읽지 못했다는 사실을 결과에 담는다.** 담지 않으면 모델이 "영상을 봤다"는 투로 답한다. 이 저장소가 네 번 잡은 실수와 같은 종류다 — 확인 안 된 것을 확인한 것처럼 보여주지 않는다.

### 20.5 못 읽었을 때 — 세 가지를 구분한다

한 덩어리로 뭉뚱그리면 모델이 껍데기를 요약해서 태연히 거짓말한다. 실측에서 쿠팡이 `Access Denied` 316자를 "본문"으로 돌려줬다.

| 상황 | 신호 | 돌려줄 말 |
|---|---|---|
| **차단** | 403·429, 또는 짧은 본문 + `Access Denied`·`captcha`·`로봇` | `그 사이트가 접근을 막았어요.` |
| **빈 페이지** | 모든 칸이 비었다 | `열리긴 했는데 읽을 내용이 없었어요. 자바스크립트로 그리는 사이트일 수 있어요.` |
| **잘림** | 상한에 걸렸다 | 본문 끝에 `…(전체 N자 중 M자만 읽었어요)` |

세 번째가 특히 중요하다. 나무위키 실측이 **37,772자**였고 상한은 3,000자다 — `…` 하나로는 "8%만 봤다"가 전달되지 않는다.

### 20.6 파일

```
lib/agent/read/extract.ts    ① ② ③ ④ ⑤ — 순수 함수. HTML 문자열이 들어가고 조각이 나온다
lib/agent/read/youtube.ts    유튜브 watch HTML → 제목·설명·챕터
lib/agent/read/budget.ts     예산과 잘림 표시
lib/agent/tools.ts           readUrl 이 위를 엮는다. <fetched-content> 액자는 지금 규칙 그대로
```

순수 함수로 떼어내는 이유는 **네트워크 없이 시험하기 위해서**다. 실제 사이트 HTML 조각을 픽스처로 박아 둔다(실측에서 이미 확보했다).

### 20.7 범위 밖

헤드리스 브라우저 렌더 · 유튜브 자막(막혔다) · 로그인이 필요한 페이지 · PDF · 사이트 전체 크롤링.
