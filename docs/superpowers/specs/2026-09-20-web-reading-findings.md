...
| 물음 | 답 |
|---|---|
| 유튜브 **자막 본문**을 받을 수 있나 | △ **집 IP 에서는 받는다**(한국어 8,942자). **배포(Vercel)에서는 못 받는다** — §2.3 |
...
| 우리가 못 읽는 페이지를 모델이 대신 읽을 수 있나 | ✅ **공급자 내장 `web_search` 가 된다**(§7) |
...
### 2.3 ⚠️ 배포 환경에서는 안 된다 — 실측

빌드 로그로 Vercel 에서 직접 재 봤다:

```
[자막탐침] watch HTTP 200, 1,571,428자
[자막탐침] INNERTUBE_API_KEY 찾음
[자막탐침] player HTTP 200
[자막탐침] 트랙 0개:
```

**요청이 막히는 게 아니라, 자막 트랙을 빼고 준다.** 같은 코드가 집 IP 에서는 31개 트랙을 받는다.

2차 탐침에서 **클라이언트 6종 × 재시도 3번**을 배포 환경에서 돌렸다. 전부 같은 답이었다:

```
ANDROID / ANDROID_VR / IOS / MWEB / WEB        → playability=LOGIN_REQUIRED
TVHTML5_SIMPLY_EMBEDDED_PLAYER                  → playability=ERROR
```

`LOGIN_REQUIRED` — 유튜브가 그 IP 에 로그인을 요구한다. **이것이 yt-dlp 가 클라우드에서
만나는 "Sign in to confirm you're not a bot" 과 같은 벽이다.** 클라이언트·도구·재시도의
문제가 아니라 IP 평판이므로 yt-dlp 로 바꿔도 같다(게다가 파이썬이라 Node 함수에서 못 돈다).
...
|---|---|
| 재시도 3번 | 6종 전부 실패 |
| 클라이언트 6종 | 전부 `LOGIN_REQUIRED`/`ERROR` |
| 브라우저에서 직접(가족의 집 IP) | **CORS 차단** — youtube.com 은 교차 출처 읽기를 허용하지 않는다 |
| 무료 프록시 6곳 | corsproxy 403 · cors.lol 429 · allorigins/codetabs 522 · thingproxy 죽음 |
| Invidious 공개 인스턴스 | 목록은 보이나 본문 0자 |
| 모바일·TV UA 로 watch 페이지(GET) | 전부 `exp=xpe`(PO 토큰 필요 표시) |

**무료로는 길이 없다.** 주거용 IP 프록시가 유일한 길이고 그건 유료다.

같은 방법을 쓴 다른 개발자도 [같은 것을 겪었다](https://velog.io/@boyfromthewell) —
"실제로 Vercel 로 배포한 이후에는 개발 환경과 달리 유튜브의 요청 차단으로 인해 데이터를 가져오는 데 실패했습니다."

**따라서 배포된 사이트에서는 자막이 아니라 설명·챕터로 답한다.** 그 사실을 결과 글이 밝힌다.
뚫으려면 주거용 IP 프록시가 필요하고 그건 유료다 — 무료 티어 방침과 충돌하므로 사용자 결정 사항.

**이 상황에서 "자막이 없습니다" 라고 말하면 거짓이다.** 있는데 안 보이는 것이므로
"확인하지 못했습니다" 로 말한다.
...
`tools` 에 `{"type":"web_search"}` 를 넣어 보냈더니 **HTTP 200**, 모델이 실제로 검색하고
`web_search_call` 이벤트를 흘린 뒤 출처까지 달아 답했다.

| 물어본 것 | 결과 |
|---|---|
| `https://example.com` 내용 | ✅ 정확히 읽고 출처 표기 |
| **쿠팡 상품 페이지**(우리 fetch 는 403) | ✅ "403 으로 막혀 확인되지 않았다"고 **정직하게** 보고 |
| 유튜브 도입부 대사 | ❌ **지어냈다.** "Hey everyone, welcome to…" 라고 답했으나 실제 첫 대사는 "Here we have a 3…" |

**그래서 순서를 정했다**: 주소를 받으면 `read_url` 을 **먼저**. 웹검색은 그것이 막혔을 때만.
그리고 웹검색으로 안 것은 **검색으로 알았다고 밝히게** 한다(시스템 안내문 규칙).

근거: `read_url` 은 우리가 받아 온 글이라 지어낼 수 없다. 웹검색은 편하지만 위 표의
세 번째 줄처럼 **본 것처럼 말하면서 틀릴 수 있다.**

실호출 확인: 쿠팡 주소를 주자 `read_url` → 차단 → 웹검색 순으로 가고,
"페이지 내용을 직접 읽지는 못했습니다. 웹검색으로도 …" 라고 출처를 갈라 답했다.
