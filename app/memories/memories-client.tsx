...
import { AGENT_NAME, byLabel } from "@/lib/agent/name";
...
export function MemoriesClient({ initial }: { initial: MemoryRow[] | null }) {
  ...
  return (
    <div>
      <PageHeader
        emoji="🧠"
        title={`${AGENT_NAME}의 기억`}
        description={`${AGENT_NAME}가 다음 대화에서도 기억하려고 적어 둔 것들이에요`}
        summary={rows.length > 0 ? `기억 ${rows.length}개` : undefined}
      />

      {/* 못 읽은 것과 없는 것은 **다른 말**이다. 섞으면 "다 잊었네" 로 읽힌다. */}
      {initial === null ? (
        ...
        <EmptyState
          emoji="🌱"
          title="아직 기억한 게 없어요"
          description={`${AGENT_NAME}에게 “이건 기억해 줘” 라고 말해 보세요.`}
        />
      ) : (
            ...
            <Card key={row.id} className="flex items-start gap-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1.5 flex items-center gap-2 text-xs text-ink-faint">
                  {/* 누가 적었는지를 **먼저** 보여 준다. 이 아이가 짐작한 것과 가족이
                      말해 준 것이 같아 보이면, 잘못 짐작한 기억이 사실처럼 굳는다. */}
                  <Tag color={row.by === "family" ? "mint" : "lavender"}>{byLabel(row.by)}</Tag>
