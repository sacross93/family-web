...
**이름은 `lib/agent/name.ts` 의 `AGENT_NAME` 한 곳에서만 온다.** 화면에 글자로 박지 말 것 —
2026-09-22 에 "포동이" → "인공 포동이" 로 바꾸면서 20개 파일 83군데를 찾아다녔다.
`포동` 은 **집이자 가족**의 이름이라 이 아이의 이름이 될 수 없다(그래서 바꿨다).
기억의 `by` 는 이름이 아니라 **열쇠**(`agent`/`family`)로 저장한다 — 이름이 또 바뀌면
옛 기억만 지난 이름을 달고 남아 "얘는 누구지?" 가 된다. `lib/agent/name.test.ts` 가 둘 다 잡는다.
...
- **공급자 오류 문구는 `lib/agent/errors.ts` 한 곳에서.** 여기서 401 은 **우리 사이트 로그인이 아니라 포동이의 ChatGPT 연결**이 끊긴 것이다(우리 세션이 풀리면 `proxy.ts` 가 로그인 화면으로 보내 이 자리까지 오지 않는다). 그런데 "로그인이 풀렸어요. 새로고침해 주세요" 를 띄우고 있었다 — **가족이 영영 안 듣는 새로고침을 반복하게 만드는 문구**다. 고칠 수 없는 일을 고치라고 시키지 말 것. 시험이 문장 자체를 잡고 있다.
- **토큰을 못 읽은 것도 401 로 올린다**(`codex.ts`). 안 올리면 라우트가 상태 없이 "잠깐 문제가 생겼어요" 를 골라 **가족이 영영 다시 시도만 한다.** `auth.ts` 의 메시지에는 `npm run agent:auth` 같은 명령어가 들어 있어 화면에 그대로 띄우면 안 된다(가족은 터미널을 쓰지 않는다) — 원문은 서버 로그에만 남고 화면에는 "연결이 풀렸어요" 가 간다.
